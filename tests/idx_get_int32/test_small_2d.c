/**
 * Checks that `idx_get_int32` will return the expected values for all points
...
int main(void) {
    ...
    idx_assert(idx_get_int32(data, 2, 0, 0) == 0x01);
    idx_assert(idx_get_int32(data, 2, 1, 0) == 0x05);
    idx_assert(idx_get_int32(data, 2, 2, 0) == 0x09);
    ...
    idx_assert(idx_get_int32(data, 2, 2, 3) == 0x0c);
    idx_assert_aborts(idx_get_int32(data, 2, 3, 3));

    idx_assert_aborts(idx_get_int32(data, 2, 0, 4));
    idx_assert_aborts(idx_get_int32(data, 2, 1, 4));
    idx_assert_aborts(idx_get_int32(data, 2, 2, 4));
    idx_assert_aborts(idx_get_int32(data, 2, 3, 4));
