/**
 * Checks that `idx_get_int32` will correctly handle arrays with bounds that
 * require multiple bytes to represent.
 */
#include "idx.h"

#include "idx_test.h"


int main(void) {
    const uint8_t header[] = {
        0x00, 0x00, 0x0c, 0x02,
        0x00, 0x00, 0x12, 0x33,
        0x00, 0x00, 0x33, 0x0f,
    };

    uint8_t *data = (uint8_t *) calloc(
        sizeof(uint8_t), 4 * 0x1233 * 0x330f + 12
    );
    idx_assert(data != NULL);

    // Copy the header.
    memcpy(data, header, 12);

    // Initialize the array with pseudo random data.
    uint32_t state = 0;
    for (size_t i = 0; i < 0x1233 * 0x330f; i++) {
        uint8_t b0 = idx_rnd(&state);
        uint8_t b1 = idx_rnd(&state);
        uint8_t b2 = idx_rnd(&state);
        uint8_t b3 = idx_rnd(&state);

        data[4 * i + 12] = b0;
        data[4 * i + 13] = b1;
        data[4 * i + 14] = b2;
        data[4 * i + 15] = b3;
    }

    // Check that `idx_get_int32` returns the expected value for every index in
    // the array.
    state = 0;
    for (size_t u = 0; u < 0x1233; u++) {
        for (size_t v = 0; v < 0x330f; v++) {
            uint8_t b0 = idx_rnd(&state);
            uint8_t b1 = idx_rnd(&state);
            uint8_t b2 = idx_rnd(&state);
            uint8_t b3 = idx_rnd(&state);

            int32_t expected = (
                (((int32_t) b0 & 0x7f) << 24) |
                (((int32_t) b1 & 0xff) << 16) |
                (((int32_t) b2 & 0xff) << 8) |
                (((int32_t) b3 & 0xff) << 0)
            );
            if (b0 & 0x80) {
                expected -= 0x80000000;
            }

            idx_assert(idx_get_int32(data, 2, u, v) == expected);
        }
        idx_assert_aborts(idx_get_int32(data, 2, u, 0x330f));
    }

    for (size_t v = 0; v <= 0x330f; v++) {
        idx_assert_aborts(idx_get_int32(data, 2, 0x1233, v));
    }

    free(data);

    return 0;
}
