/**
 * Checks that libidx can be used to save the highest possible non-denormalized
 * negative double precision floating point number.
 */
#include "idx.h"
...
int main(void) {
    const double value = ldexp(-1.0, -1022);
    ...
    idx_assert((buffer[4 + 1] & 0xf0) == 0x10);
